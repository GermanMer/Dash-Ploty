...
app.layout = html.Div([
    ...
    dcc.Graph(
        id='bar-graph'  # Gráfico de barras para la emigración
    ),
    dcc.Graph(
        id='pie-chart'  # Gráfico de torta para otra visualización
    )
])

# Definir callbacks para el gráfico de barras
@app.callback(
...
# Definir callbacks para el gráfico de torta
@app.callback(
...
def update_pie_chart(selected_year):
    ...
    # Crear un gráfico de torta basado en los datos filtrados
    fig = px.pie(filtered_migration, names=filtered_migration.index.get_level_values(0), values='Value',
                 title=f'Emigración de Argentina en el año {selected_year}',
                 labels={'names': 'Destino', 'values': 'Cantidad de Emigrantes'})
